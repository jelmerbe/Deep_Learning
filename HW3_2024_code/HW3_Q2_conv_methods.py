...
class CIFAR100Model(nn.Module):
    def __init__(self, num_outputs, p, n_channels):
        super(CIFAR100Model, self).__init__() #Specifying the model parameters 
        # input is 3x32x32
...
def train(model, data_train, lr, batch_size, device="cuda", num_epochs=200, print_freq=5):
    ...
    torch.save(model.state_dict(), f"{location}/conv_model_n_channels_{model.n_channels}_dropout_{model.p}_lr_{lr}")
    ...
    train_accuracies_series.to_csv(f"{location}/conv_train_accuracies_n_channels_{model.n_channels}_dropout_{model.p}_lr_{lr}", index=False)
