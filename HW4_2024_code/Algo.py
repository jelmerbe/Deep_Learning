...
class DQN:
    def __init__(self, obs_space, act_space, lr=1e-4, replay_size=1000000, batch_size=64,
        ...
        # choose appropriate network architecture based on the observation space
    ...
    def compute_epsilon(self):
        ...
        eps = max(0.01, 1 + (0.01 - 1) * self.num_act_steps / self.eps_decay)
        ### Your Code Ends >>>
    ...
    def train(self):
        ...
        if self.double_q:
            ...
            selected_actions = self.q_func(non_terminal_next_states).argmax(1)
            next_state_values[non_terminal_mask] = next_q_values.gather(1, selected_actions.unsqueeze(-1)).squeeze()
        else:
            next_state_values[non_terminal_mask] = next_q_values.max(1)[0]

        target_q_values = reward_batch + self.discount * next_state_values

        loss = torch.mean((target_q_values - q_values)**2)  # Hint: try using q_values, target_q_values
        ### Your Code Ends >>>
...
class ActorCritic:
    def __init__(self, obs_space, act_space, lr=0.001, nproc=1, seg_len=16, discount=0.99,
        ...
        print(f"observation shape = {obs_space.shape}, action shape = {act_space.shape}.")
