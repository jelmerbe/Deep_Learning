...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, default='dqn', choices=['dqn', 'a2c', 'ac'])
    parser.add_argument('--nproc', type=int, default=2, help='# parallel processes')
    parser.add_argument('--lr', type=float, default=0.0001, help='(critic) learning rate')
    parser.add_argument('--n_hidden_nodes', type=int, default=128, help='number of hidden nodes')
    ...
    parser.add_argument('--batch_size', type=int, default=64, help='SGD batch size')
    parser.add_argument('--discount', type=float, default=0.99, help='discount factor (or gamma)')
    parser.add_argument('--replay_size', type=int, default=10000, help='for DQN or Actor-Critic, replay buffer size')
    parser.add_argument('--need_critic_scheduler', action='store_true', help='for DQN or Actor-Critic, replay buffer size')
    ...
    if args.algo == 'dqn':
        algo = Algo.DQN(
            ...
            args.lr, args.replay_size, args.batch_size, args.discount, 
            args.target_update, args.eps_decay,
            device)
    elif args.algo == 'a2c':
        assert args.nproc * args.train_freq == args.batch_size
        algo = Algo.ActorCritic(
            env.obs_space, env.act_space,
            args.lr, args.nproc, args.train_freq, args.discount, 
            args.ent_coef, args.value_coef,
            device)
    elif args.algo == 'ac':
        algo = Algo.VanillaActorCritic(
            env.obs_space, env.act_space, args.replay_size, 
            args.batch_size, args.target_update, args.lr, 
            args.n_hidden_nodes, args.discount, args.clip, 
            args.need_critic_scheduler,
            device)
