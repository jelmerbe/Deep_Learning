    ...
    while True:
        ...
        if cmd == 'step':
            ...
            if atari:
                ...
                if t or tr: s = reset()  # override existing terminal state
...
class LocalEnv:
    def __init__(self, name, nproc, frame_skip, frame_stack, save_img_dir=None):
        ...
        for e, env in enumerate(self.envs): env.reset(seed=e)
        ...
        if self.atari:
            ...
            def reset(e):
                return self.envs[e].reset()[0].astype(np.float32)
    ...
    def step(self, actions):
        ...
        for e, a in enumerate(actions):
            ...
            if self.atari:
                ...
                r_raw, true_done = r, t
                s = s.astype(np.float32)
                if t or tr: s = self.reset_e(e)  # override existing terminal state
